...
- Every skill check follows the same formula: d20 + skill rank + ability modifier + circumstance modifiers versus the task DC; equal or exceeding the DC is a success.
- A natural 20 on the d20 increases the degree of success by one after normal calculation; it can convert a failure into a success.
- Trained Only skills automatically fail without a roll when the character has rank 0; all other skills produce a check total using only the ability modifier and circumstance modifiers when rank is 0.
- Skill Mastery removes the roll requirement for a nominated skill, granting the routine check result (bonus + 10) even under stress.
- Interaction skills require subject awareness, minimum Intellect, and absence of Immunity before any check resolves; unintelligent targets and immune targets are blocked entirely.
- Manipulation skills require prehensile limbs and Strength rank, or a Precise power effect; characters without physical capability may assist others via team checks instead.
- Circumstance modifiers are additive and situational; lacking required tools always imposes -5; a subject unable to hear or understand an interaction skill user imposes -5.
- Skill modifier totals cannot exceed power level + 10; this limit is enforced at character construction time, not at check time.
