---
name: mm3e-skill-pipeline
description: Domain-modeling pipeline for the MM3E [Skill] module. Runs domain-language, key-abstractions, abd-story-mapping pass 2, domain-sketch, and abd-acceptance-criteria with execute-rules-gate quality gates. Use when asked to run the pipeline or model the skill module.
---

# MM3E [Skill] Module Pipeline

You are a domain-modeling pipeline specialist. Process the **skill** module partition file through five ABD skills with quality gates at every step.

## Paths for this module

- **Partition file**: `c:\dev\mm3e-online-holistic\docs\modules\skill.md`
- **Output folder**: `c:\dev\mm3e-online-holistic\docs\skill\`
- **Growing module file**: `c:\dev\mm3e-online-holistic\docs\skill\skill.md`
- **Partition copy**: `c:\dev\mm3e-online-holistic\docs\skill\skill-module-partition.md`
...
1. Create folder `c:\dev\mm3e-online-holistic\docs\skill\`
2. Copy `docs/modules/skill.md` to `docs/skill/skill-module-partition.md` (read-only reference — do not modify after this step)
3. Create growing module file `docs/skill/skill.md` from the partition file content if it does not already exist
...
**Copy mode:** Write snapshot to `docs/skill/skill-domain-language.md`

**Quality gate AFTER:**
1. AI/rules pass — check output against every domain-language rule; state passes and failures
2. Scanner pass — `python c:\dev\agilebydesign-skills\skills\execute-skill-using-skills-rules\scripts\run_scanners.py --skill-root c:\Users\thoma\.cursor\skills\domain-language --workspace c:\dev\mm3e-online-holistic\docs\skill`
...
**Copy mode:** Write snapshot to `docs/skill/skill-key-abstractions.md`

**Quality gate AFTER:**
1. AI/rules pass — check output against every key-abstractions rule
2. Scanner pass — `python c:\dev\agilebydesign-skills\skills\execute-skill-using-skills-rules\scripts\run_scanners.py --skill-root c:\Users\thoma\.cursor\skills\key-abstractions --workspace c:\dev\mm3e-online-holistic\docs\skill`
3. Fix failures; re-run until clean
...
- Snapshot `docs/skill/skill-key-abstractions.md`
- Existing story map `docs/story-map.md` — identify the epic(s) that map to the skill domain
...
- Output both templates:
  - `docs/skill/story-map.md` (use template from `c:\Users\thoma\.cursor\skills\abd-story-mapping\templates\story-map.md`)
  - `docs/skill/story-map.txt` (plain-text twin with identical coverage)
...
2. Scanner pass — `python c:\dev\agilebydesign-skills\skills\execute-skill-using-skills-rules\scripts\run_scanners.py --skill-root c:\Users\thoma\.cursor\skills\abd-story-mapping --workspace c:\dev\mm3e-online-holistic\docs\skill`
3. Fix failures; re-run until clean
...
**Work:** Confirm `state: key-abstractions`. Read module file, all referenced chunks, and `docs/skill/story-map.md`. Follow the domain-sketch skill to:
...
**Copy mode:** Write snapshot to `docs/skill/skill-domain-sketch.md`

**Quality gate AFTER:**
1. AI/rules pass — check output against every domain-sketch rule
2. Scanner pass — `python c:\dev\agilebydesign-skills\skills\execute-skill-using-skills-rules\scripts\run_scanners.py --skill-root c:\Users\thoma\.cursor\skills\domain-sketch --workspace c:\dev\mm3e-online-holistic\docs\skill`
3. Fix failures; re-run until clean
...
Input context:
- Domain-sketch snapshot `docs/skill/skill-domain-sketch.md` — behaviors, interactions, invariants
- Module story map `docs/skill/story-map.md` — stories to write AC for
- All referenced source chunks
- Existing story map `docs/story-map.md` — broader context

Tasks:
- For every story in `docs/skill/story-map.md`, write WHEN/THEN/AND/BUT acceptance criteria
...
- Output both templates:
  - `docs/skill/acceptance-criteria.md` (use template from `c:\Users\thoma\.cursor\skills\abd-acceptance-criteria\templates\acceptance-criteria.md`)
  - `docs/skill/acceptance-criteria.txt` (plain-text twin with identical coverage)

**Quality gate AFTER:**
1. AI/rules pass — check output against every abd-acceptance-criteria rule
2. Scanner pass — `python c:\dev\agilebydesign-skills\skills\execute-skill-using-skills-rules\scripts\run_scanners.py --skill-root c:\Users\thoma\.cursor\skills\abd-acceptance-criteria --workspace c:\dev\mm3e-online-holistic\docs\skill`
3. Fix failures; re-run until clean
...
| File | Step |
|------|------|
| `skill-module-partition.md` | 0 |
| `skill.md` | 1-4 growing module file |
| `skill-domain-language.md` | 1 copy mode |
| `skill-key-abstractions.md` | 2 copy mode |
| `skill-domain-sketch.md` | 4 copy mode |
| `story-map.md` | 3 |
| `story-map.txt` | 3 |
